...
### 9.9 심사 기록 저장 — `review/nodes.py`, `core/paths.review_dir()` (09-10, 사용자 요청)

**문제**: Reviewer A·B 의 피드백과 Editor 의 통합 피드백이 상태에만 있다가 실행이 끝나면 사라졌다. 콘솔에는 판정 한 줄과
Editor 통합 피드백 앞 300자만 찍혀서, 실행 뒤에 "왜 2라운드에도 통과하지 못했는지" 를 확인할 방법이 없었다.

**조치**: `editor` 노드가 라운드마다 두 파일을 이번 실행 폴더에 남긴다.

- `runtime/<실행시각>/review/round_N.md` — 세 판정(A·B·코드 결정)과 세 답변의 **원문**. Editor 는 `INTEGRATED_FEEDBACK:` 으로 자르기 전
  응답 전체를 남기므로 LLM 의 `FINAL_VERDICT` 도 함께 보인다.
- `runtime/<실행시각>/review/plan_round_N.md` — 그 라운드가 심사한 계획서 본문. 재작성이 지적을 반영했는지 라운드 간 파일 비교로 볼 수 있고,
  최종본에 덮여 사라지던 **초안이 여기에만 남는다**.

`core/paths.py` 에 `review_dir()` 추가. 저장 실패는 경고만 찍고 심사를 계속한다. 콘솔 출력은 종전과 같고 저장 경로 한 줄이 늘었다.

**테스트**: `test_review_stage` 6절(판정 세 줄, 두 피드백 원문, Editor 원문, 심사 본문, 파일 두 개만 생성),
`test_pipeline_smoke`(2라운드 기록이 실행 폴더에 남고 1라운드는 REVISE, `plan_round_1.md` 가 초안과 일치).
두 테스트 모두 `paths.start_run` 으로 `tests/out/` 에 격리한다.

### 9.10 연구명이 "연구명" 으로 나오던 문제 (09-10)

**증상**: 최종본 첫 줄이 `# 연구명` 이 되어 PDF 표지 제목에 "연구명" 이 찍혔다. 실행 5건 중 2건(09-09 17:24, 09-10 13:39)에서 발생했고,
뒤의 한 건은 실제 제목이 그 아래 한 줄로 떨어져 있었다. 양식 검사는 두 건 모두 오류 0으로 통과했다.

**원인 세 가지**

1. `PROMPT_WRITER`. 9.6 에서 넣은 `[최종 출력 구조 — 제목은 이 문자열 그대로 쓴다]` 지시가 첫 줄 `# (연구명)` 에도 걸렸다.
   `## 연구 요약` 은 그대로 베껴야 하고 `# (연구명)` 은 바꿔야 하는데 한 문장이 둘 다를 지시해 모순이었다.
2. `PROMPT_WRITER_LINT_FIX` 의 마지막 줄이 **"첫 줄은 '# 연구명' 이어야 한다"** 였다. 수정 루프가 도는 실행에서는 이 지시가
   자리표시자를 되살릴 수 있었다. 문제가 난 두 실행은 오류 0이라 루프가 돌지 않았으므로 이번 증상의 원인은 아니지만 잠재 결함이었다.
3. 검사기. `연구명` 은 3자라 상한(60자) 검사에 걸리지 않았다. 제목의 **하한**도 자리표시자 대조도 없었다.
   제목 줄과 첫 `##` 사이의 줄은 파서가 어느 섹션에도 넣지 못해 버리므로, 밀려난 제목도 보이지 않았다.

**조치**

- `spec.TITLE_MIN_CHARS`(10)와 `spec.TITLE_PLACEHOLDERS` 추가. `_self_check` 가 길이 기준의 정합성을 확인한다.
- 검사 두 개 추가. `title_placeholder` 는 제목이 자리표시자이거나 하한 미만일 때, `title_stray` 는 제목 줄과 `## 연구 요약` 사이에
  본문이 있을 때 오류를 낸다. 전자의 메시지는 `lint.stray_lead()` 로 밀려난 줄을 찾아 "이 줄을 제목으로 올려라" 까지 지목하므로
  수정 루프가 한 번에 고친다.
- 프롬프트 세 개 수정. `PROMPT_WRITER` 의 출력 구조 지시를 "## 로 시작하는 제목은 그대로, 괄호 자리는 실제 내용으로" 로 나누고
  출력 규칙에 "첫 줄은 실제 제목, 아래에 반복하지 않음" 을 명시. `PROMPT_WRITER_REVIEW` 와 `PROMPT_WRITER_LINT_FIX` 에도 같은 규칙을 넣고,
  후자의 마지막 줄에서 자리표시자 요구를 제거. 검사기의 `missing_title` 메시지도 `'# 연구명'` 예시를 쓰지 않게 바꿨다.
- 테스트: `test_write_stage` 3절에 `title_placeholder` · `title_stray` 두 사례, `stray_lead`, 제목 하한, 수정 프롬프트 문구 검사를 추가.

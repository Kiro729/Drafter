...
- col: 절차 순서. 0~5 정수만 — 칸은 6개뿐입니다. 6 이상은 스키마 검증에서 즉시 거부됩니다.
  같은 lane 안에서 col 을 반복하지 마십시오. 노드의 가로 위치는 col 이, 세로 위치는 lane 이
  단독으로 정합니다. 따라서 (lane, col) 이 같은 두 노드는 완전히 겹쳐
  "workflow/node-overlap" 으로 반드시 실패하며, 수리 단계에서 되살릴 수 없습니다.
- nodes: 6~12개. 12개를 넘기지 마십시오.
  기본 골격: 데이터 출처 → 전처리·구축 → 모듈 1 → 모듈 2 → 모듈 3 → 실험·평가 → 결과.
  이 골격은 7단계인데 칸은 6개입니다. 골격을 그대로 쓰면 마지막 두 단계가 같은 col 을 쓰게 되므로,
  실험·평가와 결과를 서로 다른 lane 에 두십시오 (예: 평가 lane / 산출 lane).
  계획서의 세 모듈은 반드시 각각 노드로 두고 순서를 지키십시오.
- edges: 모든 노드가 최소 한 edge 에 나타나야 합니다. 고립 노드를 두지 마십시오.
  같은 (from, to) 쌍을 두 번 만들지 마십시오.
- mainPath: 주 경로 노드 id 배열 (2개 이상). col 이 단조 증가해야 합니다 — 같은 값은 되지만
  감소는 안 됩니다. 주 경로가 7단계 이상이면 칸이 6개뿐이라 어딘가는 col 을 반복해야 하고,
  반복하는 그 지점에서 lane 이 함께 바뀌어야 합니다.

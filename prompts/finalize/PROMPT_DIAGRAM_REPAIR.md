...
workflow/node-overlap
  같은 lane 안에서 두 노드가 같은 col 을 써서 완전히 겹쳤습니다.
  col 만 바꿔서는 풀리지 않습니다. mainPath 는 col 이 감소할 수 없고 col 상한이 5 이므로
  지목된 노드를 당겨 놓을 빈 칸이 없는 경우가 많습니다. "supportedFixes" 가 비어 있으면 그런 경우입니다.
  지목된 노드의 "lane" 을 다른 lane 으로 바꾸십시오. 마땅한 lane 이 없으면 lanes 에 하나 추가하십시오
  (최대 4개). 결과·산출물 성격의 노드라면 별도 lane 이 자연스럽습니다.
